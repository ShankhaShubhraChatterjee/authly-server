...
const sendAccountPage = async (req, res) => {
    if (req.session.auth) {
        ...
        Object.keys(clientErrors.accountErrors).forEach(
            (index) => (clientErrors.accountErrors[index] = null)
        )
        res.end()
    } else {
...
const handlePasswordUpdates = async (req, res) => {
    ...
    if (
        regexValues.oldPassword &&
        regexValues.newPassword &&
        regexValues.confirmPassword
    ) {
        let check = await matchCurrentPassword(updates.currentPassword, [
            username,
        ])
        ...
        !regexValues.oldPassword
            ? (clientErrors.accountErrors.accountCurrentPasswordError =
                  'Old Password Invalid')
            : ''
        !regexValues.newPassword
            ? (clientErrors.accountErrors.accountNewPasswordError =
                  'New Password Invalid')
            : ''
        !regexValues.confirmPassword
            ? (clientErrors.accountErrors.accountConfirmPasswordError =
...
const handleProfileImage = async (req, res) => {
    ...
    if (req.files === null) {
        ...
        imageKit
            ...
            .then((result) => {
                client.query(SQL.modifyProfileImage, [
                    result.url,
                    result.fileId,
                    user,
                ])
