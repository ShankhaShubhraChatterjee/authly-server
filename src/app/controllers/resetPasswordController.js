...
const sendResetPasswordPage = (req, res) => {
    ...
    if (
        token === req.session.reset_token &&
        email === req.session.reset_email
    ) {
...
const resetPasswordForUser = async (req, res) => {
    ...
    const passwordsMatches = matchConfirmPassword(
    ...
    setTimeout(() => {
        if (validate.newPass && validate.confirm && passwordsMatches) {
            ...
            res.redirect(
                `/user/reset-password?email=${req.session.reset_email}&token=${req.session.reset_token}`
            )
            !validate.newPass
                ? (clientErrors.resetErrors.newPassword =
                      'New Password Invalid')
                : ''
            !validate.newPass
                ? (clientErrors.resetErrors.confirmPassword =
                      'Confirm Password Invalid')
                : ''
            !passwordsMatches
                ? (clientErrors.resetErrors.passwordsMatches =
